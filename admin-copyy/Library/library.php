...
function upFile($name, $url) {
  ...
  // Kiểm tra xem đường dẫn lưu trữ tập tin có tồn tại không
  if (!file_exists($url)) {
      mkdir($url, 0777, true); // Nếu không tồn tại, tạo thư mục mới
  }
  
  // Di chuyển tập tin vào thư mục lưu trữ
  if (move_uploaded_file($tmp_name, $url. $nameIMG)) {
      return $nameIMG; // Trả về tên tập tin đã lưu trữ thành công
  } else {
      return false; // Nếu không thể di chuyển tập tin, trả về false
  }
}
